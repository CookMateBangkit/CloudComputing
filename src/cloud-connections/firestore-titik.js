...
async function tambahkanResep(documentId, ingredients, steps) {
    const data = {
        ingredients: ingredients || {},
        steps: steps || {}
    };

    await resepCollectionRef.doc(documentId).set(data);
}
...
async function masukkanDatasetCSV() {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (ingredientsString && ingredientsString.trim() !== '') {
            ...
            const docRef = db.collection('resep').doc(title);
            batch.set(docRef, { ingredients, steps });
